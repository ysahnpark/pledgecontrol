...
/**
 * Service class that provides business logic for domain
 */
class DomainServiceMongo extends BaseServiceMongo {
	 ...
	 * @return Response
	 */
	public function listDomains($criteria, $sortParams = array(), $offset = 0, $limit=100)
	{
	 ...
	 * @return Response
	 */
	public function paginateDomains($queryParams, $page_size = 20)
	{
	    // @TODO: pending
		$query = \Domain::query();
	 ...
	 */
	public function countDomains($criteria)
	{
	...
	public function createDomain($data)
	{
		...
		$validator = \Domain::validator($data);
        if ($validator->passes()) {
            $record = new \Domain();
            $record->fill($data);
	 ...
	 * @return Domain
	 */
	public function findDomain($criteria)
	{
	 ...
	 * @return Domain
	 */
	public function findDomainByPK($pk)
	{
		$criteria = array( '_id' => new \MongoId($pk) );
        return $this->findDomain($criteria);
	}
	 ...
	 */
	public function updateDomain($pk, $data)
	{
		$validator = \Domain::validator($data);
        if ($validator->passes()) {
            $record = $this->findDomainByPK($pk);
            $record->fill($data);
	 ...
	 */
	public function destroyDomain($pk)
	{
		$record = $this->findDomainByPK($pk);
		if (!empty($record)) {
		    $criteria = array( '_id' => new \MongoId($pk) );
